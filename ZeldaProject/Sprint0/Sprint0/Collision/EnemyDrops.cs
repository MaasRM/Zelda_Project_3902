    ...
    class EnemyDrops
    {
        ...
        public static void DropItem(INPC npc, List<IItem> items, Texture2D itemSheet)
        {
            ...
            if (npc is Stalfos || npc is Wallmaster)
            {
                ...
                if (determineDrop >= 25 && determineDrop < 35)
            ...
            if(npc is Goriya)
            {
                if (determineDrop < 15)
                    items.Add(new YellowRupeeItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 8 * 4, 16 * 4), new Rectangle(72, 0, 8, 16), itemSheet));
                if (determineDrop >= 15 && determineDrop < 30)
                ...
                if (determineDrop >= 30 && determineDrop < 45)
                    items.Add(new HeartItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 7 * 4, 8 * 4), new Rectangle(0, 0, 7, 8), itemSheet));
                if (determineDrop >= 45 && determineDrop < 50)
                    items.Add(new ClockItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 11 * 4, 16 * 4), new Rectangle(58, 0, 11, 16), itemSheet));
            }
            ...
            if(npc is Aquamentus)
            {
                if (determineDrop < 30)
                    items.Add(new HeartItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 7 * 4, 8 * 4), new Rectangle(0, 0, 7, 8), itemSheet));
                if (determineDrop >= 30 && determineDrop < 40)
                    items.Add(new YellowRupeeItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 8 * 4, 16 * 4), new Rectangle(72, 0, 8, 16), itemSheet));
                if (determineDrop >= 40 && determineDrop < 50)
