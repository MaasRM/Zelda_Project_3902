    ...
    class EnemyDrops
    {
        ...
        public static void DropItem(INPC npc, List<IItem> items, Texture2D itemSheet)
        {
            ...
            if (npc is Wizzrobe)
            {
                if (determineDrop < 30) items.Add(new YellowRupeeItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 8 * GameConstants.SCALE, 16 * GameConstants.SCALE), new Rectangle(72, 0, 8, 16), itemSheet));
                if (determineDrop >= 30 && determineDrop < 60) items.Add(new BlueRupeeItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 8 * GameConstants.SCALE, 16 * GameConstants.SCALE), new Rectangle(72, 16, 8, 16), itemSheet));
                if (determineDrop >= 60 && determineDrop < 90) items.Add(new HeartItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 7 * GameConstants.SCALE, 8 * GameConstants.SCALE), new Rectangle(0, 0, 7, 8), itemSheet));
                if (determineDrop >= 90 && determineDrop < 100) items.Add(new HeartContainerItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 14 * GameConstants.SCALE, 14 * GameConstants.SCALE), new Rectangle(25, 0, 14, 14), itemSheet));
            }
            ...
            if(npc is Aquamentus || npc is Gohma || npc is Dodongo) {
                items.Add(new HeartContainerItem(new Rectangle(npc.GetNPCLocation().X, npc.GetNPCLocation().Y, 7 * GameConstants.SCALE, 8 * GameConstants.SCALE), new Rectangle(0, 0, 7, 8), itemSheet));
