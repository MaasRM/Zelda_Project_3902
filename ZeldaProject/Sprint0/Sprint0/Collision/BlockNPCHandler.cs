...
namespace Sprint0
{
    public class BlockNPCHandler
    {
        ...
        public static void HandleCollision(INPC enemy, IBlock block, Rectangle overlap)
        {
            ...
            if(!(enemy is Keese)) {
                if (enemy is Trap) ((Trap)enemy).Return();
        ...
        private static OverlapInRelationToEnemy GetOverlapDirection(INPC enemy, IBlock block, Rectangle overlap)
        {
            ...
            if (overlap.Y == enemyPos.Y) overlapY = OverlapInRelationToEnemy.Up;
            else if (overlap.Y == blockPos.Y) overlapY = OverlapInRelationToEnemy.Down;

            if (overlap.X == blockPos.X) overlapX = OverlapInRelationToEnemy.Right;
            else if (overlap.X == enemyPos.X) overlapX = OverlapInRelationToEnemy.Left;
